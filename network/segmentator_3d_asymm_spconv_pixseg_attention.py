...
class UpBlock_modified(nn.Module):
    def __init__(self, in_filters, out_filters, kernel_size=(3, 3, 3), indice_key=None, up_key=None):
        super(UpBlock_modified, self).__init__()
        # self.drop_out = drop_out
        self.trans_dilao = conv3x3(in_filters, out_filters, indice_key=indice_key + "new_up")
        self.trans_act = nn.LeakyReLU()
        self.trans_bn = nn.BatchNorm1d(out_filters)
    ...
    def forward(self, x):
        ...
        upA = upA.replace_feature(upA.features)
...
class Asymm_3d_spconv(nn.Module):
    def __init__(self,
        ...
        self.upBlock3 = UpBlock_modified(4 * init_size, 2 * init_size, indice_key="up3", up_key="down2")
        self.ReconNet = ReconBlock(2 * init_size, 2 * init_size, indice_key="recon")
        self.logits = spconv.SubMConv3d(4 * init_size, nclasses, indice_key="logit", kernel_size=3, stride=1, padding=1,
                                        bias=True)


        self.upBlock3_ins_heatmap = UpBlock_modified(4 * init_size, 2 * init_size, indice_key="up3_ins_heatmap", up_key="down2")
        self.upBlock3_ins_offset = UpBlock_modified(4 * init_size, 2 * init_size, indice_key="up3_ins_offset", up_key="down2")
        self.upBlock3_ins_instmap = UpBlock_modified(4 * init_size, 2 * init_size, indice_key="up3_ins_instmap", up_key="down2")
        ...
        # breakpoint()
        self.down_add = ResBlock(2 * init_size, 2 * init_size, 0.2, height_pooling=True, indice_key="down_add2")
        self.resSeg = ResBlock(init_size, 2 * init_size, 0.2, height_pooling=True, indice_key="down_seg")  #ResBlock_seg(init_size, 4 * init_size, 0.2, height_pooling=True, indice_key="down_seg2")
        ...
        self.pool_fin = spconv.SparseConv3d(64, 64, kernel_size=3, stride=2, padding=1, bias=False)
        #####
        ...
        self.mask_attn = torch.nn.MultiheadAttention(embed_dim=64, num_heads=4, dropout=0.1,
                                                     batch_first=True)

        self.pool_attn = spconv.SparseConv3d(64, 64, kernel_size=3, stride=2,
                                        padding=1, indice_key="attn_pool", bias=False)
    ...
    def forward(self, voxel_features, coors, batch_size, pooled):
        ...
        # breakpoint()
        ret_seg1 = spconv.SparseConvTensor(feat_seg[:,0,:], coors, self.sparse_shape,batch_size)
        ret_seg2 = spconv.SparseConvTensor(feat_seg[:,1,:], coors, self.sparse_shape,batch_size)
        ret_seg3 = spconv.SparseConvTensor(feat_seg[:,2,:], coors, self.sparse_shape,batch_size)
        ...
        # _,up2e_re = self.down_add(up1e)
        
        # breakpoint()
        ####
        ret_seg1 = self.downCntxSeg(ret_seg1)
        ret_seg2 = self.downCntxSeg(ret_seg2)
        ret_seg3 = self.downCntxSeg(ret_seg3)
        ret_pix = self.downCntxPix(ret_pix)

        ######
        # breakpoint()
        down_seg2_1, _ = self.resSeg(ret_seg1)  # [2, 64, 240, 180, 16]
        down_seg2_2, _ = self.resSeg(ret_seg2)
        down_seg2_3, _ = self.resSeg(ret_seg3)
        # down_seg_1 = self.resSeg(ret_seg1)  # [2, 64, 240, 180, 16]
        # down_seg_2 = self.resSeg(ret_seg2)
        # down_seg_3 = self.resSeg(ret_seg3)
        down_pix, _ = self.resPix(ret_pix)
        #####
        # breakpoint()
        up2e = up2e.replace_feature(self.down_geo(up2e.features))
        down_seg_cat = torch.cat((down_seg2_1.features[:, None, :], down_seg2_2.features[:, None, :], down_seg2_3.features[:, None, :]), dim=1)
        down_seg, attn_w = self.mask_attn(up2e.features[:, None, :], down_seg_cat, down_seg_cat)

        # down_seg2 = attn_w[:, 0,0:1] * down_seg2_1.features + attn_w[:, 0,1:2] * down_seg2_2.features + attn_w[:, 0,2:] * down_seg2_3.features

        cat_fea = torch.cat((up2e.features[:, None, :], down_seg, down_pix.features[:, None, :]), 1)
        # down_cat,cat_w = self.fea_attn(cat_fea, cat_fea, cat_fea)
        #
        # breakpoint()
        # breakpoint()
        ...
        up1e = self.upBlock3(scomp0)
        up0e = self.ReconNet(up1e)
        up0e = up0e.replace_feature(torch.cat((up0e.features, up1e.features), 1))
        logits = self.logits(up0e)
        logits = logits.dense()
        
        up1e_ins_heatmap = self.upBlock3_ins_heatmap(scomp0)
        ...
        up1e_ins_offset = self.upBlock3_ins_offset(scomp0)
        ...
        up1e_ins_instmap = self.upBlock3_ins_instmap(scomp0)
